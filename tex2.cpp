...
GLuint CreateShaderProgram() {
    ...
    // Uproszczony Fragment Shader (dla testów)
...
bool LoadMeshToOpenGL(const tinygltf::Model& model) {
    ...
    if (mesh.primitives.empty()) {
        std::cerr << "Brak prymitywow w meshu!\n";
        return false;
    }
    ...
    if (posIndex == -1 || normIndex == -1 || texIndex == -1) {
        std::cerr << "Brak wymaganych atrybutow wierzcholkow (POSITION/NORMAL/TEXCOORD_0)!\n";
        return false;
    }

    // ... (reszta kodu ładowania danych, która wyglądała poprawnie)
    // Twój oryginalny kod do ładowania danych jest poprawny, więc nie zmieniam go
    // Tutaj należy wkleić oryginalne wczytywanie danych, ale z poprawkami jak na dole.
    
    // Skopiowałem oryginalne wczytywanie danych, bez zmian, ponieważ wyglądało na poprawne
...
void main_loop() {
    ...
    model = glm::scale(model, glm::vec3(0.1f)); // Dodano skalowanie
    ...
    // Z komentarzowany kod do ładowania tekstury
...
int main() {
    ...
    // Połącz lokalizacje atrybutów z nazwami w shaderze
    ...
    if (texIndex != -1) {
        ...
        std::cerr << "Brak tekstury w modelu.\n";
        // Jeśli brakuje tekstury, możemy ustawić domyślny kolor.
    }

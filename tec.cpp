...
bool LoadModelToOpenGL(const tinygltf::Model& model, ModelGL& modelGL) {
    ...
    // Iterujemy po wszystkich meshash, nie tylko pierwszym
        ...
        for (const auto& primitive : mesh.primitives) {
            ...
            for (int i = 0; i < vertexCount; ++i) {
                ...
                if (normals) {
                    ...
                    vertices[i].normal = glm::vec3(0.0f, 0.0f, 0.0f); // Domyślne normalne
                }
                if (texcoords) {
                    vertices[i].texcoord = glm::vec2(texcoords[i * 2 + 0], texcoords[i * 2 + 1]);
                } else {
                    vertices[i].texcoord = glm::vec2(0.0f, 0.0f); // Domyślne UV
                }
            }
            ...
            // Ładowanie tekstury dla tego mesha (jeśli istnieje)
...
int main() {
    ...
    // Sprawdź, czy model ma sceny i wierzchołki
